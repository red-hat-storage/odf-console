...
export const cleanUpVault = () => {
  cy.exec(
    `oc delete deployment ${testDeploymentJSON.metadata.name} -n default`,
    { failOnNonZeroExit: false }
  );
  cy.exec('oc delete secret ceph-csi-kms-token -n default', {
    failOnNonZeroExit: false,
  });
  cy.exec(
    'oc delete configmap csi-kms-connection-details -n openshift-storage',
    { failOnNonZeroExit: false }
  );
  cy.exec('oc delete project hashicorp', { failOnNonZeroExit: false });
};
