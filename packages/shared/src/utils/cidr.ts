...
// Overlap when every CIDR parses; unknown if any CIDR is unreadable.
export type CidrOverlapResult = 'overlap' | 'none' | 'unknown';
...
export const evaluateCidrListsOverlap = (
  ...
  return doCidrListsOverlap(left, right) ? 'overlap' : 'none';
};
