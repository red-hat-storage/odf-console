...
import { getNamespace, getOwnerReferences } from '@odf/shared/selectors';
...
const StoragePoolList: React.FC<StoragePoolListProps> = ({
  ...
  const storageClusterName = getOwnerReferences(data?.[0])?.[0]?.name;
  const createPath = `/odf/system/ns/${getNamespace(data[0])}/ocs.openshift.io~v1~StorageCluster/${storageClusterName}/storage-pools/create/~new`;
