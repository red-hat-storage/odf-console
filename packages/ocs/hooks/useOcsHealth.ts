...
export const computeOCSHealth = (
  ...
  const cephCluster = getResourceInNs(ceph.data, systemNamespace);
  const cephObjectStore = getResourceInNs(cephObj.data, systemNamespace);
  const noobaaCluster = getResourceInNs(noobaa.data, systemNamespace);
