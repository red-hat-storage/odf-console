...
export const useStorage = (
  ...
  React.useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === S3_CREDENTIALS_LOCAL_STORE_KEY && e.newValue) {
        try {
          ...
          console.error('Error launching modal', err);
        }
      }
    };
  ...
  const setSecretRefWithStorage = React.useCallback(
    (value: SecretRef, targetStorageType: StorageType) => {
      if (isAdmin) return;
