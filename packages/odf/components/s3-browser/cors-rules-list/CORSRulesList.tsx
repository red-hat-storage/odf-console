...
const RuleRow: React.FC<RowProps<CORSRule, CustomData>> = ({
  ...
  const allowedHeadersRowValue = isAllowAllConfig(allowedHeaders)
    ? t('All headers')
    : allowedHeadersLength + t(' allowed');
  // exposed-headers
  const exposedHeadersLength = corsRuleObj?.ExposeHeaders?.length ?? 0;
  const exposedHeadersRowValue = exposedHeadersLength + t(' exposed');
  ...
  const detailsPagePath = `${BUCKETS_BASE_ROUTE}/${bucketName}/permissions/cors/details?${getSearchParam(ruleName, ruleHash)}`;
