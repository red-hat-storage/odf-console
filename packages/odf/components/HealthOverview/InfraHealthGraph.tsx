...
export const InfraHealthGraph: React.FC<InfraHealthGraphProps> = ({
  ...
  // Zoom state for controlled zoom - uses Victory's DomainTuple for x-axis
    ...
    (domain: VictoryZoomDomain) => {
      if (domain?.x) {
        ...
        if (onZoomDomainChange) {
          // Convert Victory's DomainTuple to our Date-based ZoomDomain
    ...
    <div ref={ref} className="pf-v6-u-mt-sm odf-infra-health-graph">
      {zoomDomain && (
        <div className="odf-infra-health-graph__header">
          ...
          >
            {t('Viewing: {{start}} - {{end}}', {
              start: (zoomDomain.x[0] instanceof Date
                ? zoomDomain.x[0]
                : new Date(zoomDomain.x[0])
              ).toLocaleTimeString(),
              end: (zoomDomain.x[1] instanceof Date
                ? zoomDomain.x[1]
                : new Date(zoomDomain.x[1])
              ).toLocaleTimeString(),
            })}
          </Label>
          <Button
