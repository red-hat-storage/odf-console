  ...
  const switchStorageCluster = React.useCallback(() => {
    if (storageClusters?.length > 1) {
      const currentIndex = storageClusters.findIndex(
        (cluster) => cluster.metadata.name === selectedCluster.clusterName
      );
      const nextIndex = (currentIndex + 1) % storageClusters.length;
      const nextCluster = storageClusters[nextIndex];
      setSelectedCluster({
        clusterName: nextCluster.metadata.name,
        clusterNamespace: nextCluster.metadata.namespace,
        isExternalMode: isExternalCluster(nextCluster),
      });
    }
  }, [storageClusters, selectedCluster]);

  const memoizedHasMultipleStorageClusters = React.useMemo(
    () => storageClusters?.length > 1,
  ...
  return noStorageClusters || selectedCluster.clusterName === '' ? (
    ...
    <div>
      ...
      <OCSDashboardContext.Provider
        ...
        value={{
          hasMultipleStorageClusters: memoizedHasMultipleStorageClusters,
          selectedCluster,
        }}
      >
        {/* eslint-disable-next-line react/jsx-no-constructed-context-values */}
        <OCSDashboardDispatchContext.Provider value={{ switchStorageCluster }}>
          <OCSSystemDashboard />
