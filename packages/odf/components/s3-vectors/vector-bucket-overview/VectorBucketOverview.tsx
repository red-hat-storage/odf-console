  ...
  return [
    {
      ...
      isAriaDisabled: hasVectorIndexes,
      onClick: () =>
...
const VectorBucketOverview: React.FC<{}> = () => {
  ...
  const renderActions =
    (s3VectorsClient: S3VectorsCommands, hasVectorIndexes: boolean) => () =>
...
const VectorBucketOverviewContent: React.FC<BucketOverviewContentProps> = ({
  ...
  const { data: listIndexesData, isLoading: isIndexesCheckLoading } = useSWR(
    s3VectorsClient && vectorBucketName
      ? [
          'vector-bucket-delete-guard-indexes',
          vectorBucketName,
          s3VectorsClient.providerType,
          fresh,
        ]
      : null,
    () =>
  ...
  const hasVectorIndexes =
    !isIndexesCheckLoading && listIndexesData?.indexes?.length > 0;
  ...
  return (
    <>
      <PageHeading
        ...
        actions={actions(s3VectorsClient, hasVectorIndexes)}
