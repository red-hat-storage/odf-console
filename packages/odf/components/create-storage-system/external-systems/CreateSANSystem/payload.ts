...
export const createLocalDisks = async (
  ...
  const localDisks: LocalDiskKind[] = disks.map((disk) => {
    if (!disk.WWN || !disk.path || !disk.nodeName) {
      throw new Error(
        `Invalid disk data: missing required fields (WWN: ${disk.WWN}, path: ${disk.path}, nodeName: ${disk.nodeName})`
      );
    }
    return {
  ...
  try {
    ...
    throw new Error(
      `Failed to create LocalDisks: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
};
...
export const createLocalFileSystem = async (
  ...
  if (!fsName || fsName.trim() === '') {
    throw new Error('FileSystem name is required');
  }
  if (!localDisks || localDisks.length === 0) {
    throw new Error(
      'At least one LocalDisk is required for FileSystem creation'
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to create FileSystem: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
};
...
export const createStorageClass = async (
  ...
  if (!fileSystem) {
    throw new Error('FileSystem is required for StorageClass creation');
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to create StorageClass: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
};
