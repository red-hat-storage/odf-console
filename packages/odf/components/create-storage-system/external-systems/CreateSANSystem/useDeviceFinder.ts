...
const getSharedDiscoveredDevicesRepresentatives = (
  ...
  // Map to first device from each shared group (already unique by WWN due to grouping)
    ...
    if (!_.isEmpty(memoizedSelectedNodes)) {
      ...
      if (selectedHostnames.length > 0) {
        ...
        consoleFetch(
          ...
          console.error('Failed to update device finder:', error);
