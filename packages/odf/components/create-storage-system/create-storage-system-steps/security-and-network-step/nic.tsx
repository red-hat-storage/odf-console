...
/** Get mon-ip from node (supports both WizardNodeState and NodeKind) */
const getMonIp = (node: {
  annotations?: Record<string, string>;
  metadata?: { annotations?: Record<string, string> };
}): string | undefined =>
  node.annotations?.[MON_IP_ANNOTATION]?.trim?.() ||
  node.metadata?.annotations?.[MON_IP_ANNOTATION]?.trim?.();
