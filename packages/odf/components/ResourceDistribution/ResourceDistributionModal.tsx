...
import { getProvisioner, isCephProvisioner } from '@odf/shared/storage-classes';
import {
  WatchK8sResources,
...
import { generatePatchForDistributionOfResources } from './utils';
...
const StorageClassRowGenerator: React.FC<
...
> = ({ closeModal, isOpen, extraProps: { resource } }) => {
  ...
  const isVolumeSnaphostClassDataLoaded = data?.volumeSnapshotClasses?.loaded;
  ...
  React.useEffect(() => {
    if (
      ...
      isVolumeSnaphostClassDataLoaded &&
  ...
  return (
    <Modal
      className="odf-modal"
      title={'Manage distribution of storage classes'}
      isOpen
      onClose={closeModal}
      open={isOpen}
    >
      <Tabs activeKey={activeTabKey} onSelect={handleTabClick}>
        ...
        >
          <ResourceDistributionTable
            ...
            loaded={isVolumeSnaphostClassDataLoaded}
