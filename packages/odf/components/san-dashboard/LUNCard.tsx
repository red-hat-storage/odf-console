...
const getLUNGroupStatus = (_fileSystem: FileSystemKind): string => {
  if (isConnected(_fileSystem)) {
    return 'Healthy';
  }
  return 'Unhealthy';
};
...
// Helper function to get storage class name from filesystem
// This would need to be enhanced to query StorageClasses that reference this filesystem
const getStorageClassName = (fileSystem: FileSystemKind): string => {
  // For now, return the filesystem name as storage class name
  // This should be enhanced to query StorageClasses that use this filesystem
  return fileSystem.metadata.name;
};

// Helper function to get console link from RemoteCluster
// This would need to be enhanced to query RemoteCluster referenced by the filesystem
const getConsoleLink = (_fileSystem: FileSystemKind): string | undefined => {
  // For now, return undefined
  // This should be enhanced to query RemoteCluster and get GUI host from spec.remote.cluster
  return undefined;
};
...
const lunGroupStatusFilter = (t): RowFilter<FileSystemKind> => ({
  ...
  items: [
    { id: 'Healthy', title: t('Healthy') },
    { id: 'Unhealthy', title: t('Unhealthy') },
  ],
  ...
  return (
    <>
      <TableData {...tableColumnInfo[0]} activeColumnIDs={activeColumnIDs}>
        <Link
          to={`/odf/external-systems/scale.spectrum.ibm.com~v1beta1~cluster/SAN_Storage/filesystems/ns/${obj.metadata.namespace}/${obj.metadata.name}`}
        >
          {obj.metadata.name}
        </Link>
      </TableData>
