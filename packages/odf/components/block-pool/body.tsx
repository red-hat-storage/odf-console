...
import { CEPH_STORAGE_NAMESPACE, OCS_DEVICE_REPLICA } from '../../constants';
...
import { checkArbiterCluster } from '../../utils';
import {
  PROGRESS_STATUS,
  ProgressStatusProps,
  BlockPoolState,
...
export const BlockPoolBody = (props: BlockPoolBodyPros) => {
  ...
  >(OCSStorageClusterModel, null, CEPH_STORAGE_NAMESPACE);
  ...
  }, [cephCluster, dispatch, isPoolManagementSupported, setVolumeType, state.volumeType]);
  ...
  const replicaDropdownItems = replicaList.map((replica) => (
    ...
    >
      {t('{{replica}} Replication', { replica: OCS_DEVICE_REPLICA[replica] })}
    </DropdownItem>
  ));
