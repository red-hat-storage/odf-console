...
export const LUN_FIRST_GROUP_CREATION_TIMEOUT_MS = 30 * 60 * 1000;
...
export const getLUNGroupStatus =
  (isFirstFileSystem: (creationTimestamp: string) => boolean) =>
  (fileSystem: FileSystemKind): HealthState => {
    ...
    if (isLunGroupCreating(fileSystem)) {
      ...
      if (creationTimestamp) {
        ...
        if (
          isFirstFileSystem(creationTimestamp)
            ? Date.now() - createdAt > LUN_FIRST_GROUP_CREATION_TIMEOUT_MS
            : Date.now() - createdAt > LUN_GROUP_CREATION_TIMEOUT_MS
        ) {
