...
const CapacityAutoscalingModal: React.FC<CapacityAutoscalingModalProps> = ({
  ...
  // If the CR doesn't exist it only means autoscaling is disabled.
  let canEdit =
    isCapacityAutoScalingAllowedInDay2 &&
    (alreadyEnabled || storageAutoScalerLoadError?.code === 404);
  let status = '';
      ...
      <ModalBody>
        {canEdit ? (
          ...
          <Alert
            ...
            variant={AlertVariant.danger}
            title={t(
              'Smart Scaling is available only for non local dynamic storage.'
            )}
          >
            {t('Local dynamic storage is present in the cluster.')}
          </Alert>
        ) : (
...
const CapacityAutoscalingActionModal: React.FC<
  CapacityAutoscalingActionModalProps
> = ({ extraProps: { storageCluster }, ...props }) => {
  return (
    <CapacityAutoscalingModal storageCluster={storageCluster} {...props} />
  );
};
