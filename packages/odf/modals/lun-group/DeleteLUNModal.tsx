...
const DeleteLUNModal: React.FC<DeleteLUNModalProps> = ({
  ...
  const [deletionStatus, setDeletionStatus] = React.useState<DeletionStatus>(
    DeletionStatus.LOADING
  );
  const [boundPVCNames, setBoundPVCNames] = React.useState<string[]>([]);
  ...
  React.useEffect(() => {
    if (scLoaded && pvcLoaded && !scLoadError) {
      // If storage class doesn't exist, we can proceed with deletion
      if (!storageClass) {
        setDeletionStatus(DeletionStatus.ALLOWED);
        return;
      }
      ...
      if (usedByPVCs) {
        ...
        const pvcNames = pvcResources.items
          ...
          .map((pvc) => getName(pvc));
        setBoundPVCNames(pvcNames);
        setDeletionStatus(DeletionStatus.BOUNDED);
      } else {
        setDeletionStatus(DeletionStatus.ALLOWED);
      }
    }
  }, [scLoaded, pvcLoaded, storageClass, pvcResources, fsName, scLoadError]);
