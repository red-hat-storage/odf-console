...
describe('ConfigureDFSelections', () => {
  ...
  it('should navigate to MCG setup when MCG card is clicked', async () => {
    ...
    const mcgCard = container.querySelector('#mcg') as Element;
    await user.click(mcgCard);

    expect(mockConsoleLog).toHaveBeenCalledWith('Redirecting to ', 'mcg');
    expect(mockNavigate).toHaveBeenCalledWith(`${CREATE_SS_PAGE_URL}?mode=mcg`);
    expect(mockCloseModal).toHaveBeenCalled();
  });
...
describe('Integration Tests', () => {
  ...
  it('should handle multiple card clicks correctly', async () => {
    ...
    // Click external system card
    const externalSystemCard = document.body.querySelector(
      '#external-system'
    ) as Element;
    await user.click(externalSystemCard);
    ...
    // Reset mocks and simulate another modal instance
    jest.clearAllMocks();

    // Click MCG card
    const mcgCard = document.body.querySelector('#mcg') as Element;
    await user.click(mcgCard);
