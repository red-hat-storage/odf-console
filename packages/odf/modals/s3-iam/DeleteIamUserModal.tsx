...
const getDescription = (t: TFunction) => (
  <div className="text-muted">
    {t(
      'Delete the IAM user permanently? All user data, accesskeys, secret keys and inline policies related to this user will also be deleted'
    )}
  </div>
);
...
> = ({
  ...
  const refetch = async () => {
    await Promise.all([refetchUser(), refetchAccessKeys()]);
  };
          ...
          >
            <Text>
              {t('IAM user has {{count}} active access keys', {
                count: iamAccessKeys.filter((ak) => ak.Status === 'Active')
                  .length,
              })}
            </Text>
