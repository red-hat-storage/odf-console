...
describe('evaluateSubmarinerPrePair', () => {
  ...
  it('reports Unknown when Available is True but ConnectionDegraded is absent', () => {
    expect(
      evaluateSubmarinerPrePair([
        cluster(addonWithConditions([{ type: 'Available', status: 'True' }])),
        cluster(healthyAddon()),
      ])
    ).toEqual({ canProceed: false, status: SubmarinerStatus.Unknown });
  });

  it('reports Unknown when Available condition is missing', () => {
    expect(
      evaluateSubmarinerPrePair([
        cluster(
          addonWithConditions([
