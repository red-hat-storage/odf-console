...
export const useDRPoliciesByClusterPair = (): [
  ...
  const clusterPairPoliciesMap: ClusterPairPoliciesMap = {};
  ...
  if (loaded && !loadError && drPolicies) {
    drPolicies.forEach((policy) => {
      ...
      const isConfiguring = CONFIGURING_PHASES.includes(phase);
      ...
      if (!clusterPairPoliciesMap[pairKey]) {
        clusterPairPoliciesMap[pairKey] = [];
      }
      clusterPairPoliciesMap[pairKey].push(policyInfo);
    });
  }

  return [clusterPairPoliciesMap, loaded, loadError];
};
