...
const ProtectionTypeReview: React.FC<{
  virtualMachine: AssignPolicyViewState['virtualMachine'];
  appType: DRApplication;
}> = ({ virtualMachine: { protectionName, protectionType }, appType }) => {
...
const DataPolicyReview: React.FC<{ policy: DRPolicyType }> = ({ policy }) => {
  const { t } = useCustomTranslation();
      ...
      {modalType === ModalType.VirtualMachine ? (
        <>
          <ProtectionTypeReview
            ...
            appType={appType}
          />
          <DataPolicyReview policy={state.policy} />
        </>
      ) : (
        <>
          <DataPolicyReview policy={state.policy} />
