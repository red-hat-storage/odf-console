...
import { DRPlacementControlModel, VirtualMachineModel } from '@odf/mco/models';
import {
  getName,
  RowComponentType,
  SelectableTable,
    ...
    <>
      ...
      <Td dataLabel={getSharedVMGroupsColumns(t)[1].columnName}>
        <StatusIconAndText
          ...
          title={placementControlInfo.protectedVMNames.length.toString()}
        />
      </Td>
...
export const SharedVMGroupsTable: React.FC<SharedVMGroupsTableProps> = ({
  ...
  const setSharedVMGroupInfo = (selectedRows: DRPlacementControlType[]) => {
    ...
    dispatch({
      ...
      payload: {
        policy: drPolicy,
        k8sSyncInterval: drpc.lastKubeObjectProtectionTime,
        protectionName: drpc.vmSharedGroupnName,
        sharedVMGroup: drpc.protectedVMNames,
      },
    });
  };
...
const getSharedVMGroupColumns = (t: TFunction<string>) => [
  { columnName: t('Name'), sortFunction: (a, b, c) => sortRows(a, b, c) },
];
...
// Main component for displaying the table
export const SharedVMGroupTable: React.FC<SharedVMGroupTableProps> = ({
  sharedVMGroup,
}) => {
  const { t } = useCustomTranslation();
  ...
  return (
    <Table
      ...
      rawData={sharedVMGroup.protectedVMNames as []}
...
// Type definitions
type SharedVMGroupTableProps = { sharedVMGroup: DRPlacementControlType };
