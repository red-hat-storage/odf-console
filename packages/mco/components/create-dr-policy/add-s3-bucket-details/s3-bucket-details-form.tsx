...
> = ({
  ...
  const validate = (cluster: 1 | 2, details: S3Details) => {
    ...
    if (!details.endpoint.trim()) errs.endpoint = 'Endpoint is required';
    if (!details.accessKeyId.trim())
      errs.accessKeyId = 'Access key ID is required';
    if (!details.secretKey.trim()) errs.secretKey = 'Secret key is required';
    if (!details.region.trim()) errs.region = 'Region is required';
    if (!details.s3ProfileName.trim())
      errs.s3ProfileName = 'S3 profile name is required';
  ...
  const fields: Array<{ key: keyof S3Details; label: string }> = [
    ...
    { key: 's3ProfileName', label: 'S3 profile name' },
  ];
