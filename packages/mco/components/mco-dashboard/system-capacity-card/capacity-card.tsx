...
import { StorageDashboard, CAPACITY_QUERIES } from '@odf/mco/components/mco-dashboard/queries';
...
import { PrometheusResult, PrometheusResponse, useK8sWatchResource } from '@openshift-console/dynamic-plugin-sdk';
...
const getClusterURL = (clusterClaimsList: ClusterClaimObject[]) =>
  clusterClaimsList?.find((claimObj: ClusterClaimObject) => claimObj?.name === CLUSTER_CLAIM_URL_NAME)?.value || undefined;
...
const systemNameSort = (a: CapacityMetricDatum, b: CapacityMetricDatum, c: SortByDirection) => {
  const negation = c !== SortByDirection.asc;
  const sortVal = a.systemName.localeCompare(b.systemName);
  return negation ? -sortVal : sortVal;
};

const clusterNameSort = (a: CapacityMetricDatum, b: CapacityMetricDatum, c: SortByDirection) => {
  const negation = c !== SortByDirection.asc;
...
const SystemCapacityCard: React.FC = () => {
  ...
  const [managedClusters, managedClustersLoaded, managedClustersError] = useK8sWatchResource<ACMManagedClusterKind[]>({
  ...
  const [totalCapacity, errorTotalCapacity, loadingTotalCapacity] =
    useURLPoll<PrometheusResponse>(`${ACM_ENDPOINT}=${CAPACITY_QUERIES[StorageDashboard.TOTAL_CAPACITY_FILE_BLOCK]}`);

  React.useEffect(() => {
    const ManagedClusterLink: ManagedClusterLinkMap = managedClustersLoaded && !managedClustersError
      ? managedClusters?.reduce((acc: ManagedClusterLinkMap, cluster: ACMManagedClusterKind) => {
        acc[cluster?.metadata?.name] = getClusterURL(cluster?.status?.clusterClaims);
        return acc;
      }, {} as ManagedClusterLinkMap)
      : {} as ManagedClusterLinkMap;

    const dataMap: CapacityMetricDatumMap = !loadingUsedCapacity && !errorUsedCapacity
      ? (usedCapacity?.data?.result?.reduce((acc: CapacityMetricDatumMap, usedMetric: PrometheusResult) => {
        ...
        const clusterURL = ManagedClusterLink.hasOwnProperty(clusterName)
          ? ManagedClusterLink[clusterName] : undefined;
    ...
    !loadingTotalCapacity && !errorTotalCapacity && totalCapacity?.data?.result?.forEach((totalMetric: PrometheusResult) => {
      const dataMapKey = totalMetric?.metric?.storage_system + totalMetric?.metric?.cluster;
    ...
    const data: CapacityMetricDatum[] = _.map(dataMap, (metricDatumMap: CapacityMetricDatum) => metricDatumMap);
