...
import { TOTAL_PVC_COUNT_QUERY } from '@odf/mco/components/mco-dashboard/queries';
import {
...
export const PVCsSection: React.FC<PVCsSectionProps> = ({
  ...
  const { t } = useCustomTranslation();
  const [pvcsCount, pvcsCountError, pvcsCountLoading] = useCustomPrometheusPoll(
    {
      endpoint: 'api/v1/query' as any,
      query: TOTAL_PVC_COUNT_QUERY,
      basePath: ACM_ENDPOINT,
      cluster: HUB_CLUSTER_NAME,
    }
  );

  const [totalPVCsCount, protectedPVCsCount] = React.useMemo(() => {
    let totalPVCsCount = '0';
    if (!pvcsCountLoading && !pvcsCountError) {
      totalPVCsCount = pvcsCount?.data?.result?.find(
