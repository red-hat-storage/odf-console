...
const filterItems = (name: string, searchValue: string) => name.toLowerCase().includes(searchValue.toLowerCase());

const subsFilter = (drPlacementControls: DRPlacementControlKind[], subscription: ACMSubscriptionKind) => {
    const placementRuleNameMapping: PlacementRuleNameMapping = drPlacementControls?.reduce((arr, drpc) => ({ ...arr, [drpc?.metadata?.namespace]: [...(arr[drpc?.metadata?.namespace] || []), drpc?.spec?.placementRef?.name]}), {});
    return  (
        subscription?.spec?.placement?.placementRef?.kind === ACMPlacementRuleModel?.kind &&
        placementRuleNameMapping?.[subscription?.metadata?.namespace]?.includes(subscription?.spec?.placement?.placementRef?.name)
    );
};

const fetchApplications = (subscriptionMapping: SubcsriptionMapping, app: ApplicationKind): string[]  => {
    let appNames: string[] = [];
...
export const ApplicationStatus: React.FC<ApplicationStatusPros> = ((props) =>{
    ...
    const[applicationMapping, setApplicationMapping] = React.useState<ApplicationMapping>({});
    const[subscriptionMapping, setSubscriptionMapping] = React.useState<SubcsriptionMapping>({});
    ...
    const [subscriptions, subscriptionsLoaded, subscriptionsLoadError] = useK8sGet<ListKind<ACMSubscriptionKind>>(
    ...
    React.useEffect(() => {
        if(applicationsLoaded && !applicationsLoadError) {
            ...
            const appMapping: ApplicationMapping = applications?.items?.reduce((arr, application) => appFilter(application) ? { ...arr, [application?.metadata?.namespace]: {...arr[application?.metadata?.namespace],  [application?.metadata?.name]: application}} : arr, {});
            setApplicationMapping(appMapping);
        };

        if(subscriptionsLoaded && !subscriptionsLoadError){            
            // namespace wise subscription object
            const subsMapping: SubcsriptionMapping = subscriptions?.items?.reduce((arr, subscription) => (subsFilter(drPlacementControls, subscription)? { ...arr, [subscription?.metadata?.namespace]: {...arr[subscription?.metadata?.namespace],  [subscription?.metadata?.name]: subscription}} : arr), {});
        ...
        if(Object.keys(applicationMapping).length !== 0 && Object.keys(subscriptionMapping).length !== 0) {
            ...
            Object.keys(applicationMapping).forEach((namespace) => {
                Object.keys(applicationMapping[namespace]).forEach((name) => {
                    appNames = [...appNames, ...fetchApplications(subscriptionMapping, applicationMapping[namespace][name])];
                });
            });
    ...
    const onSearch = (searchValue: string) => {
        ...
        if (searchValue === '') {
            ...
            setFilteredNames(applicationNames.filter(name => filterItems(name, searchValue)));
        }
    };
                    ...
                    <ToolbarContent>
                        <ToolbarItem>
                            <SearchInput
                                ...
                                type="text" aria-label={t("application name search")}
                                value={searchAppName}
                                onChange={onSearch}
