...
const fetchMirrorPeer = (mirrorPeers: MirrorPeerKind[], peerNames: string[]): MirrorPeerKind => (
  mirrorPeers?.find((mirrorPeer) => {
    const existingPeerNames = mirrorPeer?.spec?.items?.map((item) => item?.clusterName) ?? [];
...
const SelectedCluster: React.FC<SelectedClusterProps> = ({
  ...
  const { name: clusterName, region, storageSystem, odfVersion} = cluster ?? {};
  ...
  const [cephCluster, cephClusterLoaded, cephClusterLoadError] = useK8sGet<K8sResourceKind>(
    CephClusterModel,
    CEPH_CLUSTER_NAME,
    CEPH_STORAGE_NAMESPACE,
    clusterName
  ...
  React.useEffect(() => {
    if(ssLoaded && cephClusterLoaded && csvListLoaded && !(ssLoadError && cephClusterLoadError && csvListLoadError)){
  ...
  return (
    <Flex display={{ default: 'inlineFlex'}} className="mco-create-data-policy__flex">
      <FlexItem>
        <Badge key={id} isRead>
          {id}
        </Badge>
...
export const CreateDRPolicy: React.FC<ReRouteResourceProps> = ({match, history}) => {
  ...
  const [selectedClusters, setSelectedClusters] = React.useState<ManagedClusterMapping>({});
  const [isReplicationOpen, setReplicationOpen] = React.useState(false);
  const [isReplicationInputManual, setIsReplicationInputManual] = React.useState(false);
  ...
  const clustersData =  React.useMemo(() => Object.values(selectedClusters), [selectedClusters]);

  React.useEffect(() => {
    if (clustersData.length === 2) {
      // ODF detection
      setODFDetected(clustersData.every((cluster) => (cluster?.storageSystem !== "" && cluster?.isValidODFVersion)));
      // DR replication type
      const isReplicationAutoDetectable = clustersData?.every((cluster) => cluster?.storageClusterId !== "");
      ...
      setReplication(isReplicationAutoDetectable && storageClusterIDs.size  <= 1 ? REPLICATION_TYPE(t)['sync'] : REPLICATION_TYPE(t)['async']) 
    } else{
  ...
  const replicationDropdownItems = Object.values(REPLICATION_TYPE(t)).map((replType) => (
    <DropdownItem
  ...
  const onCreate = () => {
    ...
    const mirrorPeer: MirrorPeerKind  = fetchMirrorPeer(mirrorPeers, peerNames) ?? {};
     ...
     if(Object.keys(mirrorPeer).length > 0) {
      ...
      if (replication === REPLICATION_TYPE(t).async) {
        const patch = [{
          ...
          value: [...new Set([...mirrorPeer?.spec?.schedulingIntervals, syncTime])],
        }];
        promises.push(
          k8sPatch({model: MirrorPeerModel, resource: mirrorPeer, data: patch})
        );
      };
    } else {
      // MirrorPeer creation
      const payload: MirrorPeerKind = {
        ...
        spec: {
          ...
          schedulingIntervals: replication === REPLICATION_TYPE(t)['async'] ? [syncTime] : [],
      ...
      promises.push(
        k8sCreate({ model: MirrorPeerModel, data: payload })
      );
    };
    ...
    const payload: DRPolicyKind = {
      ...
      spec: {
        schedulingInterval: replication === REPLICATION_TYPE(t)['async'] ? syncTime : "0m",
    ...
    Promise.all(promises).then(() => {
      ...
      const drPolicyKind = referenceForGroupVersionKind(apiGroup)(apiVersion)(kind);
      ...
      <Form className="mco-create-data-policy">
        ...
        {isODFDetected ? (
          ...
          clustersData?.map((c) => (
            c.csvLoaded && !c?.isValidODFVersion ? (
              <Alert 
                title={t('{{ name }} has invalid ODF version, update to ODF {{ version }} or latest version to enable DR protection', { name: c?.name, version: ODF_MINIMUM_SUPPORT})}
                variant="danger"
                isInline
              />
            ) :
            c?.storageSystem === '' && c.storageSystemLoaded && (
              <Alert 
                title={t('{{ name }} is not connected to RHCS', { name: c?.name})}
                variant="danger"
                isInline
              />
            )
        ...
        <ActionGroup className="mco-create-data-policy__action-group">
          <Button
            ...
            onClick={onCreate}
            isDisabled={
              !policyName || Object.keys(selectedClusters)?.length !== MAX_ALLOWED_CLUSTERS || !isODFDetected
            }
          >
            {t('Create')}
          </Button>
